...
def make_header():
  ...
  seq += 1
  time = Time()
  time.time = current_milli_time();
  header.stamp = time
  header.frame_id = "0"
  return header

def read_gyros():
  return read_mult(gyro)

def read_accels():
  return read_mult(accel)

def read_mult(addrs, error_index = 0):
  result = Vector3()
  vals = []
...
def talker():
  ...
  while not rospy.is_shutdown():
    ...
    sample.angular_velocity = read_gypos()
    sample.linear_acceleartion_covariance = [0]*9
